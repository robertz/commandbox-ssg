<cfcomponent hint="Compiles Java source dirs to an array of .jar files." output="false">
	...
	<cffunction name="init" hint="Constructor" access="public" returntype="JavaCompiler" output="false">
		<cfargument
			name    ="jarDirectory"
			hint    ="the directory to build the .jar file in, defaults to ./tmp"
			type    ="string"
			required="No"
		...
		try {
			data.compiler = getPageContext()
				.getClass()
				.getClassLoader()
				.loadClass( defaultCompiler )
				.newInstance();
		} catch ( any exc ) {
		...
		if ( NOT structKeyExists( data, "compiler" ) ) {
			throwException(
				"javaCompiler.NoCompilerAvailableException",
	...
	<cffunction
		name      ="compile"
		hint      ="compiles Java to bytecode, and returns a JAR"
		access    ="public"
		returntype="any"
		output    ="false"
	>
		...
		<cfargument
			name    ="classLoader"
			hint    ="a optional URLClassloader to use as the parent for compilation"
			type    ="any"
			required="false"
		>
		<cfargument
			name    ="jarName"
			hint    ="The name of the jar file. Defaults to a UUID"
			type    ="string"
			required="false"
			default ="#createUUID()#.jar"
		>
		<cfscript>
		// setup file manager with default exception handler, default locale, and default character set
		var fileManager = getCompiler().getStandardFileManager(
			javacast( "null", "" ),
			javacast( "null", "" ),
			javacast( "null", "" )
		);
		...
		<cfloop array="#arguments.directoryArray#" index="directoryToCompile">
			...
			if ( structKeyExists( arguments, "classLoader" ) ) {
				options = addClassLoaderFiles(
					options,
					arguments.classLoader,
					arguments.directoryArray
				);
			}
		...
		if ( NOT compilePass ) {
			throwException(
				"javacompiler.SourceCompilationException",
	...
	<cffunction
		name      ="addClassLoaderFiles"
		hint      ="adds a set of files to the file manager from the urlclassloader"
		access    ="private"
		returntype="array"
		output    ="false"
	>
		...
		<cfargument
			name    ="classLoader"
			hint    ="URLClassloader to use as the parent for compilation"
			type    ="any"
			required="true"
		>
		<cfargument name="directoryArray" hint="array of directories to compile" type="array" required="Yes">
