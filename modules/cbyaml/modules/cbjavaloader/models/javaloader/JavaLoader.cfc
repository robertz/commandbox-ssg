...
<cfcomponent name="JavaLoader" hint="Loads External Java Classes, while providing access to ColdFusion classes">
	...
	<cffunction name="init" hint="Constructor" access="public" returntype="JavaLoader" output="false">
		<cfargument
			name    ="loadPaths"
			hint    ="An array of directories of classes, or paths to .jar files to load"
			type    ="array"
			default ="#arrayNew( 1 )#"
			required="no"
		>
		<cfargument
			name    ="loadColdFusionClassPath"
			hint    ="Loads the ColdFusion libraries"
			type    ="boolean"
			required="No"
			default ="false"
		>
		<cfargument
			...
			hint    ="(Expert use only) The parent java.lang.ClassLoader to set when creating the URLClassLoader"
			type    ="any"
			default =""
			required="false"
		>
		<cfargument
			...
			hint    ="Directories that contain Java source code that are to be dynamically compiled"
			type    ="array"
			required="No"
		>
		<cfargument
			name    ="compileDirectory"
			hint    ="the directory to build the .jar file for dynamic compilation in, defaults to ./tmp"
			type    ="string"
			required="No"
			default ="#getDirectoryFromPath( getMetadata( this ).path )#/tmp"
		>
		<cfargument
			name    ="trustedSource"
			hint    ="Whether or not the source is trusted, i.e. it is going to change? Defaults to false, so changes will be recompiled and loaded"
			type    ="boolean"
			required="No"
			default ="false"
		>
	...
	>
		...
		try {
			...
			throwException(
				...
				"The requested class '#arguments.className#' could not be found in the loaded jars/directories."
			);
		}
		</cfscript>
	</cffunction>
	...
	<cffunction
		...
		hint      ="Returns the com.compoundtheory.classloader.NetworkClassLoader in case you need access to it"
		access    ="public"
		returntype="any"
		output    ="false"
	>
	...
	<cffunction
		name="switchThreadContextClassLoader"
		hint="Sometimes you will need to switch out the ThreadContextClassLoader with the classloader used by JavaLoader.<br/>
			It has :
	...
	>
		...
		<cftry>
			...
			<cfelse>
				<cfthrow
					type   ="javaloader.InvalidInvocationException"
					message="Unable to determine what method to invoke"
					detail ="Please check the documentation for switchThreadContextClassLoader."
				/>
			</cfif>
	...
	<cffunction
		name      ="getVersion"
		hint      ="Retrieves the version of the loader you are using"
		access    ="public"
		returntype="string"
		output    ="false"
	>
	...
	<cffunction
		name      ="createWithSourceCheck"
		hint      ="does the create call, but first makes a source check"
		access    ="private"
		returntype="any"
		output    ="false"
	>
	...
	<cffunction
		name      ="loadClasses"
		hint      ="loads up the classes in the system"
		access    ="private"
		returntype="void"
		output    ="false"
	>
	...
	<cffunction
		name      ="calculateJarName"
		hint      ="returns the jar file name for a directory array"
		access    ="private"
		returntype="string"
		output    ="false"
	>
	...
	<cffunction
		name      ="calculateSourceLastModified"
		hint      ="returns what the source last modified was"
		access    ="private"
		returntype="date"
		output    ="false"
	>
		...
		<cfloop from="1" to="#len#" index="counter">
			...
			if ( qLastModified.recordCount ) {
				...
				if ( dateCompare( lastModified, qlastModified.dateLastModified ) eq -1 ) {
					...
					*/
					lastModified = createObject( "java", "java.util.Date" ).init(
						createObject( "java", "java.io.File" )
							.init( qLastModified.directory & "/" & qLastModified.name )
							.lastModified()
					);
				}
			} else {
	...
	<cffunction
		name      ="ensureNetworkClassLoaderOnServerScope"
				...
				if ( NOT structKeyExists( server, key ) ) {
					while ( iterator.hasNext() ) {
						Array.set(
							urls,
							counter,
							createObject( "java", "java.io.File" ).init( iterator.next() ).toURL()
						);
						counter = counter + 1;
					}
	...
	<cffunction
		name      ="createJavaProxy"
		hint      ="create a javaproxy, dependent on CF server settings"
		access    ="private"
		returntype="any"
		output    ="false"
	>
	...
	<cffunction
		name      ="queryJars"
		hint      ="pulls a query of all the jars in the /resources/lib folder"
		access    ="private"
		returntype="array"
		output    ="false"
	>
	...
	<cffunction
		name      ="getServerURLClassLoader"
		hint      ="returns the server URL class loader"
		access    ="private"
		returntype="any"
		output    ="false"
	>
	...
	<cffunction
		name      ="hasJavaCompiler"
		hint      ="whether this object has a javaCompiler"
		access    ="private"
		returntype="boolean"
		output    ="false"
	>
	...
	<cffunction
		name      ="hasSourceLastModified"
		hint      ="whether this object has a sourceLastModified"
		access    ="private"
		returntype="boolean"
		output    ="false"
	>
